...
def infer(args, multiple=False):
    ...
    for img in args.imgs:
        ...
        if reps == None:
                ...
                if multiple:
                    if confidence >= 0.2:
                        print("Predict {} @ x={} with {:.2f} confidence.".format(person.decode('utf-8'), bbx,
                                                                         confidence))
                        tagList.append(person.decode('utf-8'))
                else:
            ...
            imgInfo = {}
            imgInfo['ext'] = os.path.splitext(img)[1]
            imgInfo['tags'] = tagList
            tagged_img_Dic[os.path.splitext(img)[0]] = imgInfo

            doc = {}
            doc['_id'] = ObjectId()
            doc[os.path.splitext(img)[0]] = imgInfo
            if len(tagList) > 0:
                col.insert(doc)

    return(tagged_img_Dic)
...
#imgs = ['/none/sehwa/group-image-set/Brad_Pitt/000022.jpg']    
    ...
    imgs = []    
#    begin_img = args.imgs[0]
#    end_img = args.imgs[1]

    begin = int(args.imgs[0])
    end = int(args.imgs[1])

#    flag = True
    pathName = "/none/sehwa/group-image-set/"
#    for dirName in os.listdir(pathName):
    for i in range(begin,end):
        dirName = os.listdir(pathName)[i]
        print dirName
        fullDirName = os.path.join(pathName, dirName)
        for fileName in os.listdir(fullDirName):
            fullFileName = os.path.join(fullDirName, fileName)

            if os.path.splitext(fileName)[1].lower() == '.png' or\
            os.path.splitext(fileName)[1].lower() == '.jpg' or\
...
#                    imgs.append(fullFileName)                
    ...
    tagged_img_Dic = infer(args, args.multi)


'''            
            if fullFileName == begin_img:
                flag = False
            if fullFileNmae == end_img:
                break
'''
