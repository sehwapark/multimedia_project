...
db = conn['multimedia']
col = db['relationship_c20'] 


def getRep(align, net, args, imgPath, multiple=False):
    start = time.time()
...
def infer(align, net, args, multiple=False):
    ...
    for img in args.imgs:
        tagList = {}
        print("\n=== {} ===".format(img))
        reps = getRep(align, net, args, img, multiple)
        if reps == None:
            continue
        else:
            if len(reps) > 1:
...
#                    tagList.append(person.decode('utf-8'))
                    tagList[person.decode('utf-8')] = confidence
                else:
        ...
        for x,y in sorted_rels:
            if i > 10:
                ...
                while True:
                    ...
                    if not line: break
                    imgList.append(line)
                relation['relationship'] = doc['relationship'][x]['relationship']
